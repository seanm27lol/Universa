#!/usr/bin/env python3
"""Sealed runner for experiment ``universa-router-group-sealed-1``.

The frozen design: one :class:`universa.router.StructureRouter`
(``feature_dim=18``, ``hidden_dim=64``) trained on the 18-dim degree-2
NO-ANCHOR degradation profiles of group-nerve switch instances
(``family='z6_to_z3'``, Z/6 -> Z/3 by reduction mod 3 through the bar
construction, ``num_decoys=2``) over the train seed block 21001..21200 x
operating fractions {0.2, 0.3, 0.4, 0.5} (one replicate, index 0, per
row; 800 rows), then compared on the sealed eval seed block
40101..40136 x held-out fractions {0.6, 0.7, 0.8} against TWO baselines:
the polluted argmin observed-residual oracle (argmin of the
operating-fraction degree-2 residual column) and, descriptively, the
non-learned grid-mean ``log1p`` degree-2-profile heuristic. Every
observation is a :class:`universa.partial_group.GroupObservationModel`
draw with ``corrupt_fraction = g`` at grid point ``g`` (a nested prefix
of one master permutation of the ``n^2`` table entries has its observed
product replaced by a different uniformly-drawn element); the profile
grid 0.2..0.9 by 0.1 EXCLUDES 0.0 — no clean column exists anywhere.
Each data row (seed, fraction, replicate) gets
``observation_seed = subseed(seed, "sealed-replicate", str(fraction),
str(replicate_index))``, driving the nested corruption draw
(``group-observe``, ``corrupt``); the SAME seed feeds all arms within a
row (exactly paired), with R=4 replicates per eval (seed, fraction).
The per-(seed, fraction) candidate permutation is
...
FULL degree-2 residual profile — the degree-2 observed nerve
commutation residual at all 8 grid points, 8 raw floats in permuted
candidate order — plus the operating-fraction degree-2 residual. The
...
learned router integrates the full degree-2 degraded profile; the
oracle reads only one polluted column (the operating fraction's
degree-2 residual). The heuristic baseline in H4 is the profile-shape
...
(``docs/17-router-group-seal.json``) have been committed and pushed.
...
EXPERIMENT_ID = "universa-router-group-sealed-1"
RESULT_SCHEMA = "universa-router-group-sealed-result/1"
...
PROTOCOL = "docs/16-sealed-router-group-protocol.md"
RUNNER_SOURCE = "scripts/run_group_sealed_1.py"
DEFAULT_SEAL = "docs/17-router-group-seal.json"
SEAL_SCHEMA = "universa-seal/6"

PROTOCOL_SHA256 = (
    "d974f4b47b16699273149276cd792cf1fdf1d06764d32c236d4086eda4939ea6"
)
...
# Frozen task family: group-nerve switch instances, Z/6 -> Z/3 by reduction
# mod 3 through the bar construction, K = 3 candidates with the true target
# at index 0 pre-permutation.
FAMILY = "z6_to_z3"
SOURCE_ORDER = 6
TARGET_ORDER = 3
NUM_DECOYS = 2
...
TRAIN_SEEDS = tuple(range(21001, 21201))  # 21001..21200
SEALED_EVAL_SEEDS = tuple(range(40101, 40137))  # 40101..40136
...
FEATURE_DIM = 18  # 8 degree-2 profile + 7 slopes + 3 structural dims
HIDDEN_DIM = 64
EPOCHS = 150
LEARNING_RATE = 1e-3
TORCH_SEED = 4242
...
# H1..H3, ``d_heur`` (learned minus the non-learned grid-mean degree-2
# profile heuristic) for H4.
_CLAIM_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        "id": "h1-group-0.7-primary",
        "fraction": 0.7,
        "threshold": 0.0,
        "role": "primary",
        "baseline": "oracle",
        ...
        "description": (
            ...
            "oracle (operating-fraction degree-2 column) at the held-out "
            "fraction 0.7"
        ),
    },
    {
    ...
    },
    {
        ...
        "description": (
            ...
            "degree-2 profile heuristic at the held-out fraction 0.7"
        ),
    },
)
...
DECLARED_CAVEAT = (
    ...
    "the full degree-2 degraded profile; the oracle reads only one "
    "polluted column (the operating fraction's degree-2 residual). The "
...
class _ReplicateRow:
    ...
    ``features`` is the permuted (K, FEATURE_DIM) float64 degree-2
    no-anchor degradation-profile block exactly as
    :func:`group_candidate_features` computes it (per-candidate ``log1p``
    of the degree-2 observed nerve commutation residual over the 0.2..0.9
    grid under nested table corruption, first-difference slopes,
    structural dims). ``residual_profiles`` holds, per candidate, the raw
    degree-2 profile (the residual at all grid points, 8 floats, permuted
    order); ``operating_residuals`` the per-candidate degree-2 residual at
    the operating fraction — the column the polluted oracle reads. All
    derive from ONE shared observation draw (the same seed drives the
    nested corruption draw of every candidate), so the arms are exactly
    paired.
    """
...
def _replicate_observation_seed(seed: int, fraction: float, replicate: int) -> int:
    ...
    the nested corruption draw (``group-observe``, ``corrupt``) of every
    :class:`universa.partial_group.GroupObservationModel` the row
    constructs.
    """
    if replicate < 0:
...
def _feature_names(profile_grid: Any = PROFILE_GRID) -> tuple[str, ...]:
    ...
    ``log1p`` of the degree-2 observed nerve commutation residual at every
    grid point, then the first-difference slopes (named by the grid
    fractions they span), then the structural dims — the target group
    order, the morphism count ``N1``, and the composable-pair count
    ``N2`` — read off the observed
    nerve (corruption never changes shapes).
    """
...
def group_candidate_features(
    ...
    """Degree-2 no-anchor feature vector for one group-nerve candidate.

    Returns ``(features, residuals)``: the FEATURE_DIM-wide float64 feature
    vector laid out as :func:`_feature_names` plus the RAW degree-2
    residuals, so the row builder can retain the full degree-2 profile (the
    retention contract: both non-learned arms recompute from raw rows
    alone). The feature profile itself is the degree-2 residual per grid
    point — the frozen score of the protocol.

    The candidate's group is recovered from its nerve (fail-closed:
    :func:`universa.partial_group._group_from_nerve` rejects any input that
    is not the nerve of a group) and observed through
    :class:`universa.partial_group.GroupObservationModel` draws with
    ``corrupt_fraction = g`` for every grid fraction ``g``, under the
    shared ``observation_seed`` — one draw per (row, grid point), reused
    across candidates. Every candidate has the same order, so the
    corruption sweep is nested per fraction for every candidate (prefixes
    of one master permutation of the table entries, replacements drawn in
    the same order). Every profile entry is ``log1p`` of the degree-2
    :func:`universa.partial_group.observed_nerve_commutation_residual` of
    the planted induced map against the observed candidate — polluted at
    every grid point, with no exact anchor anywhere.

    The structural dims (target group order, morphism count ``N1``,
    composable-pair count ``N2``) are read off the OBSERVED nerve of the
    row's sweep (corruption never changes shapes): every feature is
    ...
    if candidate.top_degree != 2:
        raise ValueError("routing candidates are group nerves (2-complexes)")
    probe = ChainMap(chain_map.source, candidate, chain_map.maps)
    group = _group_from_nerve(candidate)
    observations = [
        GroupObservationModel(
            group, observation_seed, corrupt_fraction=g
        ).observe()
        for g in grid
    ]
    residuals = tuple(
        float(observed_nerve_commutation_residual(probe, observed))
        for observed in observations
    )
    profile = np.log1p(np.asarray(residuals, dtype=np.float64))
    slopes = np.diff(profile)
    first = observations[0]
    group_order = first.truth_order
    num_morphisms = first.d1.shape[1]
    num_pairs = first.d2.shape[1]
...
def _undegraded_misfits(instance: Any) -> tuple[float, ...]:
    """Per-candidate degree-2 clean commutation residual.
    ...
    — it never enters features (every feature is computed against a
    degraded operator). Degree 1 is structurally zero for group nerves
    (one object on both sides: ``d1' N1 = 0 = N0 d1``), so degree 2 is THE
    audit degree.
    """
    misfits = []
    for candidate in instance.candidates:
        ...
        if len(residuals) != 2:
            raise DesignFailureError(
                ...
                f"(a group nerve, levels 0-2), got {len(residuals)}",
                seed=instance.seed,
            )
        misfits.append(float(residuals[1]))
...
def _build_replicate_row(
    ...
    *,
    ...
    degree-2 profile for the heuristic, and the operating-fraction degree-2
    residuals for the oracle, from ONE shared observation draw.

    The feature block is computed exactly as
    :func:`group_candidate_features` computes it — per candidate under the
    row's shared observation seed with the nested corruption sweep over the
    0.2..0.9 grid — permuted by the per-(seed, fraction) permutation from
    ``subseed(seed, "router-v2-permutation", f"{fraction:.6f}")``
...
def _build_train_block() -> tuple[np.ndarray, np.ndarray]:
    ...
    seed at degree 2; any violation is a whole-run design failure, never a
    dropped row.
    """
    ...
    for seed in TRAIN_SEEDS:
        ...
        if misfits[0] > AUDIT_TOL:
            raise DesignFailureError(
                f"train seed {seed}: true candidate degree-2 misfit "
        ...
        if decoy_floor <= AUDIT_TOL:
            raise DesignFailureError(
                ...
                "degree 2; ground truth is not separable"
            )
...
def _oracle_index(operating_residuals: Any) -> int:
    ...
    degree-2 residual column (log1p is strictly increasing, so this is
...
def _heuristic_index(residual_profiles: Any) -> int:
    ...
    uniform grid-mean of the ``log1p`` degree-2 residual profile — the
    simplest whole-trajectory integration with no learning and no
    operating-fraction choice. Reads only the retained raw degree-2
    profiles (8 floats per candidate)."""
...
def _score_row(
    ...
    argmin over the per-candidate degree-2 residuals at the operating
    fraction (the row's profile column; log1p is strictly increasing, so
    this is exactly argmin of the degree-2 misfit). Heuristic
    (descriptive, non-learned): argmin over candidates of the uniform
    grid-mean of the ``log1p`` degree-2 profile. All three read the same
...
def _recompute_paired(raw_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    operating-fraction degree-2 residuals and the heuristic arm's bits
    from the retained full degree-2 profiles (8 floats per candidate) —
    ...
    *,
    ...
    block: dict[str, Any] = {
        "recompute_scope": (
            ...
            "degree-2 residuals; the heuristic arm's per-row bits are "
            "recomputed from the retained per-row full degree-2 residual "
            "profiles (the degree-2 observed commutation residual at all 8 "
            "grid points, 8 floats per candidate) — BOTH non-learned arms "
...
def _design_record() -> dict[str, Any]:
    return {
        ...
        "task_family": (
            "group-nerve switch instances via "
            "universa.category_instances.make_group_switch_instance("
            "family='z6_to_z3', num_decoys=2) (Z/6 -> Z/3 by reduction "
            "mod 3 through the bar construction); K=3 candidates, true "
            "target at index 0 pre-permutation"
        ),
        "observation_degradation": (
            "universa.partial_group.GroupObservationModel with "
            "corrupt_fraction = g at every grid point (a nested prefix of "
            "one master permutation of the n^2 table entries has its "
            "observed product replaced by a different uniformly-drawn "
            "element); the profile grid 0.2..0.9 by 0.1 excludes 0.0 — no "
            "clean column exists anywhere (the no-anchor regime, lifted to "
            "group nerves)"
        ),
        "feature": (
            "the 18-dim degree-2 no-anchor degradation-profile feature "
            "vector exactly as this runner's group_candidate_features "
            "computes it: log1p of the degree-2 observed nerve commutation "
            "residual at the 8 grid points under the row's observation "
            "draw (8 columns), the 7 first-difference slopes, and 3 "
            "structural dims (target group order, morphism count N1, "
            "composable-pair count N2) read off the observed nerve"
        ),
        ...
        "replicate_subseed": (
            ...
            "str(replicate_index)) per data row, driving the nested "
            "corruption draw (group-observe, corrupt); the same "
            "observation_seed feeds all arms within a row (paired)"
        ),
        "permutation": (
        ...
        "raw_row_retention": (
            "each eval raw row retains, per candidate, the full degree-2 "
            "residual profile (the degree-2 observed commutation residual "
            "at all 8 grid points; 8 raw floats, permuted candidate "
            "order) plus the operating-fraction degree-2 residual, so the "
            "oracle AND heuristic arms' per-row bits are recomputable "
            "from the raw rows alone (the audit block recomputes them); "
            "only the learned arm needs the retained hash-pinned model"
        ),
        "training": {
        ...
        "baselines": {
            "oracle": (
                ...
                "operating-fraction degree-2 residual column (polluted at "
                "every fraction — no clean column exists)"
            ),
            "heuristic": (
                "descriptive non-learned heuristic: argmin of the "
                "grid-mean log1p degree-2 profile (the profile-shape "
                "comparator of H4)"
            ),
        },
        ...
        "eligibility_rule": (
            ...
            "instance passes at degree 2 (true misfit <= 1e-9, every "
            "decoy > 1e-9); deterministic per seed and never entering "
            "features"
        ),
...
def run(project_root: Path, output: Path, *, seal: str = DEFAULT_SEAL) -> dict[str, Any]:
    ...
    try:
        for seed in SEALED_EVAL_SEEDS:
            ...
            if misfits[0] > AUDIT_TOL:
                problems.append(
                    f"true candidate degree-2 misfit {misfits[0]:.3e} on "
                    f"the undegraded instance exceeds {AUDIT_TOL}"
                )
            for index, value in enumerate(misfits[1:]):
                if value <= AUDIT_TOL:
                    problems.append(
                        f"decoy {index} misfit {value:.3e} on the "
                        f"undegraded instance is at or below {AUDIT_TOL} "
                        "at degree 2; ground truth is not separable"
                    )
            seed_records.append(
                {
                    "seed": seed,
