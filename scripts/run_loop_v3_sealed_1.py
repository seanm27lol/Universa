#!/usr/bin/env python3
"""Sealed runner for experiment ``universa-loop-v3-sealed-1``.

The frozen design: the DEGRADED-regime route-or-discover loop of
:mod:`universa.loop_v2` under the ALARM-V2 redesign — routing and the
fit/no-fit alarm are LEARNED in the no-anchor regime (every candidate
boundary observed through :class:`universa.partial.ObservationModel`
draws with ``mask_fraction = 0.25`` AND corruption swept over the
0.2..0.9 profile grid that excludes 0.0, so no residual the loop reads is
exact), the alarm is the :class:`universa.loop_v2.LearnedAlarmV2` over
the :func:`universa.loop_v2.alarm_features_v2` layout (the frozen v1
columns plus five margin features) with its decision threshold CALIBRATED
on the train block by :func:`universa.loop_v2.calibrate_threshold`
(``max_false_quiet_rate = 0.02``), while discovery stays certified on the
exact transported observations. THREE models are trained inside the
sealed run on the train seed block 180001..180400 (never on a sealed eval
seed) and the alarm threshold is calibrated on the same block, then FOUR
arms are compared on the sealed eval seed block 190001..190036 x THREE
...
:func:`universa.loop_v2.calibrate_threshold` with
``max_false_quiet_rate = 0.02`` (the threshold maximizing balanced
accuracy subject to the false-quiet bound) — the full six-key calibration
record is retained in the training provenance; the
...
(``docs/26-router-loop-v3-seal.json``) have been committed and pushed. It
...
EXPERIMENT_ID = "universa-loop-v3-sealed-1"
RESULT_SCHEMA = "universa-router-loop-v3-sealed-result/1"
...
PROTOCOL = "docs/25-sealed-router-loop-v3-protocol.md"
RUNNER_SOURCE = "scripts/run_loop_v3_sealed_1.py"
DEFAULT_SEAL = "docs/26-router-loop-v3-seal.json"
SEAL_SCHEMA = "universa-seal/9"

PROTOCOL_SHA256 = (
    "5151bbb9ea561c0ab46a6e16e2d07b498cd10d3123fe36985f0bdd7c2468b869"
)
...
TRAIN_SEEDS = tuple(range(180001, 180401))  # 180001..180400
SEALED_EVAL_SEEDS = tuple(range(190001, 190037))  # 190001..190036
...
"""The frozen false-quiet bound of the train-block threshold calibration.

The v2 alarm's decision threshold is picked on the train block by
:func:`universa.loop_v2.calibrate_threshold`: the threshold maximizing
balanced accuracy subject to ``false_quiet_rate <= MAX_FALSE_QUIET_RATE``
(a false quiet — the alarm staying quiet on an out-of-library row, so no
discovery runs and the acquisition is lost — was the sealed loop-v2
experiment's dominant error mode).
"""
...
_CLAIM_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        "id": "h1-loopv3-arch-vs-generic-e2e",
    ...
    {
        "id": "h2-loopv3-arch-vs-routing-only-e2e",
    ...
    {
        "id": "h3-loopv3-arch-vs-generic-inlibrary",
    ...
    {
        "id": "h4-loopv3-arch-vs-discovery-only-inlibrary-harm",
...
# rule). Schema universa-seal/9 claim objects carry ``statistic`` (the loop
...
def _validate_calibration_record(record: Any) -> None:
    """Fail-closed shape check of the frozen six-key calibration record.

    The record is what :func:`universa.loop_v2.calibrate_threshold`
    returns; the runner re-validates it before the threshold is allowed
    anywhere near an eval row: exactly the six frozen keys, the threshold
    and the three rates finite floats in ``[0, 1]``, ``bound_satisfied`` a
    bool, and ``num_candidates`` an int of at least 2 (the sweep always
    includes the 0.0 and 1.0 candidates).
    """
    expected = {
        "threshold",
        "balanced_accuracy",
    ...
    if not isinstance(record, dict) or set(record) != expected:
        raise DesignFailureError(
            "the calibration record must carry exactly the six frozen "
    ...
    if not isinstance(record["bound_satisfied"], bool):
        raise DesignFailureError(
            "the calibration record's bound_satisfied must be a bool"
        )
...
def _train_models(
    ...
    calibrate the v2 alarm's decision threshold on the same train rows
    under the frozen false-quiet bound, and record every model state
    SHA-256, the final training scalars, and the full six-key calibration
    record.
    ...
    # 2b. The frozen train-block calibration: the v2 alarm's decision
    # threshold is picked on the SAME train rows — the threshold
    # maximizing balanced accuracy subject to false_quiet_rate <=
    # MAX_FALSE_QUIET_RATE (0.02). Deterministic (no RNG anywhere).
    try:
        calibration = calibrate_threshold(
            alarm,
            rows_fit,
            rows_nofit,
            max_false_quiet_rate=MAX_FALSE_QUIET_RATE,
        )
    except DesignFailureError:
        raise
    except Exception as error:
    ...
    provenance = {
        ...
        "alarm": {
            ...
            "calibration_bound": MAX_FALSE_QUIET_RATE,
            "calibration_rule": (
                "universa.loop_v2.calibrate_threshold on the SAME train "
                "rows: the threshold maximizing balanced accuracy subject "
                "to false_quiet_rate <= max_false_quiet_rate (0.02), "
                "ties toward the smaller false-quiet rate then the "
                "larger threshold; if no candidate satisfies the bound, "
                "the fallback records bound_satisfied False"
            ),
        },
        "generic": {
...
def _descriptive_block(
    ...
    return {
        ...
        "v1_vs_v3_baseline": {
            "note": (
                "the documented loop-v2 baseline of docs/24, quoted "
                "verbatim: the v1 arch arm's per-condition accuracies "
                "(in-library 0.972, out-of-library 0.833, null-control "
                "1.000), its 61/108 discovery invocations (1 in-library "
                "+ 30 out-of-library + 30 null-control) against "
                "discovery-only's 108/108, and the -1/36 bounded-harm "
                "estimate on seed 160031. descriptive only, read by no "
                "decision rule."
            ),
...
def _design_record() -> dict[str, Any]:
    return {
        ...
        "training": {
            ...
            "calibration": (
                "the v2 alarm's decision threshold is calibrated on the "
                "train block by universa.loop_v2.calibrate_threshold "
                "with max_false_quiet_rate = 0.02 (the threshold "
                "maximizing balanced accuracy subject to the false-quiet "
                "bound); the full six-key record (threshold, "
                "balanced_accuracy, false_quiet_rate, false_alarm_rate, "
                "bound_satisfied, num_candidates) is retained in the "
                "training provenance"
            ),
            "generic": (
        ...
        "descriptive": (
            ...
            "calibration record, and the v1-vs-v3 tradeoff comparison "
            "against the published docs/24 baseline (the v1 arch arm's "
            "per-condition accuracies 0.972/0.833/1.000, its 61/108 "
            "discovery invocations against discovery-only's 108/108, "
            "and the -1/36 bounded-harm estimate on seed 160031) — the "
            "price of selectivity, read by no decision rule"
        ),
        "eligibility_rule": (
