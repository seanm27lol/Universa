#!/usr/bin/env python3
"""Sealed runner for experiment ``universa-loop-v2-sealed-1``.

The frozen design: the DEGRADED-regime route-or-discover loop of
:mod:`universa.loop_v2` — routing and the fit/no-fit alarm are LEARNED in
the no-anchor regime (every candidate boundary observed through
:class:`universa.partial.ObservationModel` draws with ``mask_fraction =
0.25`` AND corruption swept over the 0.2..0.9 profile grid that excludes
0.0, so no residual the loop reads is exact), while discovery stays
certified on the exact transported observations. THREE models are trained
inside the sealed run on the train seed block 170001..170200 (never on a
sealed eval seed), then FOUR arms are compared on the sealed eval seed
block 160001..160036 x THREE paired conditions. Each eligible seed
contributes TWELVE raw rows — one per (condition, arm):
...
* **arms** — ``arch_full`` (learned router + learned alarm + certified
  discovery), ``routing_only`` (learned router, no alarm, no discovery —
  a forced choice), ``discovery_only`` (always certified discovery, the
  library unused for routing), and ``generic`` (the no-architecture
...
:class:`universa.loop_v2.LearnedAlarm` (seed 4243) trains on
fit/no-fit labels from the in-library vs out-of-library rows (the gates
of the trained router, tau = 1.0); the
...
order is pinned — router (4242), alarm (4243), generic (4244) — because
the alarm's features read the trained router's soft gates. Every model
state's SHA-256 and the final training scalars are recorded.
...
**Determinism, verified.** Every train row, every eval row, and every
model hash is a deterministic function of the frozen seeds: the runner
executes every eval (seed, condition, arm) row TWICE and requires
bit-identical rows — every field except the retained
...
alarm's fit/no-fit, and the generic model's class are NOT recomputable
from the rows — the three learned models are hash-pinned in
...
no-anchor degraded regime, with small MLPs trained on 200 train seeds
...
(``docs/23-router-loop-v2-seal.json``) have been committed and pushed. It
...
EXPERIMENT_ID = "universa-loop-v2-sealed-1"
RESULT_SCHEMA = "universa-router-loop-v2-sealed-result/1"
...
PROTOCOL = "docs/22-sealed-router-loop-v2-protocol.md"
RUNNER_SOURCE = "scripts/run_loop_v2_sealed_1.py"
DEFAULT_SEAL = "docs/23-router-loop-v2-seal.json"
SEAL_SCHEMA = "universa-seal/8"

PROTOCOL_SHA256 = (
    "31493402eeb31ed7af6aa9d96fde894be6ef4c453cdd456702080eba1454f494"
)
...
TRAIN_SEEDS = tuple(range(170001, 170201))  # 170001..170200
SEALED_EVAL_SEEDS = tuple(range(160001, 160037))  # 160001..160036
...
_CLAIM_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        "id": "h1-loopv2-arch-vs-generic-e2e",
    ...
    {
        "id": "h2-loopv2-arch-vs-routing-only-e2e",
    ...
    {
        "id": "h3-loopv2-arch-vs-generic-inlibrary",
    ...
    {
        "id": "h4-loopv2-arch-vs-discovery-only-inlibrary-harm",
...
# rule). Schema universa-seal/8 claim objects carry ``statistic`` (the loop
...
DECLARED_CAVEAT = (
    ...
    "regime, with small MLPs trained on 200 train seeds of the same "
...
class _TrainBlock:
    """The runner-built train block (200 train seeds x both views).
    ...
    router rows), and the LearnedAlarm rows are computed from both views
    once the router is trained (the alarm reads the trained router's
    gates). ``generic_features``/``generic_labels`` are the GenericMLP's
    rows: both views' generic spectral blocks, in-library labeled 0 (the
    true candidate's index under the unpermuted loop convention) and
    out-of-library labeled K (no-fit).
    """
...
def _execution_environment() -> dict[str, Any]:
    ...
    return {
        "compute": (
            ...
            "(StructureRouter, LearnedAlarm, GenericMLP); numpy float64 for "
...
def _train_models(
    ...
    order — StructureRouter (seed 4242), LearnedAlarm (seed 4243, reading
    the trained router's gates), GenericMLP (seed 4244) — and record every
    model state SHA-256 and the final training scalars.
    ...
    # 2. The LearnedAlarm: fit/no-fit labels from the in-library vs
    ...
    # sees them at eval time.
    try:
        rows_fit = np.stack(
            [
                alarm_features(router_gates(router, in_block), in_raw)
                for in_block, in_raw in zip(block.in_blocks, block.in_raws)
            ]
        )
        rows_nofit = np.stack(
            [
                alarm_features(router_gates(router, out_block), out_raw)
                for out_block, out_raw in zip(block.out_blocks, block.out_raws)
            ]
        )
        alarm, alarm_history = train_alarm(
            lambda: LearnedAlarm(NUM_VIEW_CANDIDATES),
            rows_fit,
            rows_nofit,
            epochs=EPOCHS,
            lr=LEARNING_RATE,
    ...
    provenance = {
        ...
        "alarm": {
            ...
            "input": (
                "the K soft router gates (tau = 1.0), the gate entropy "
                "(nats), log1p of the minimum raw profile value, and "
    ...
    return router, alarm, generic, provenance
...
def _descriptive_block(raw_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    *,
    ...
    block: dict[str, Any] = {
        "recompute_scope": (
            ...
            "gates, the alarm's fit/no-fit, and the generic model's class "
    ...
    if complete:
        ...
        block["claims"] = claims
        block["descriptive"] = _descriptive_block(raw_rows)
    return block
    ...
    return {
        ...
        "arms": {
            "arch_full": (
                ...
                "LearnedAlarm decides fit/no-fit from the soft gates and "
                "the raw profile block; fit routes to the argmax, no-fit "
        ...
        "training": {
            ...
            "alarm": (
                "LearnedAlarm(K=3, hidden_dim=32) via "
                "universa.loop_v2.train_alarm, torch seed 4243, 150 "
                "epochs, lr 1e-3, threshold 0.5; fit/no-fit labels from "
                "the in-library vs out-of-library rows, reading the "
                "trained router's soft gates (tau = 1.0)"
            ),
            "generic": (
        ...
        "descriptive": (
            ...
            "and wall-time summaries per (condition, arm) and per arm — "
            "the price of selectivity, read by no decision rule"
        ),
        "eligibility_rule": (
...
def run(project_root: Path, output: Path, *, seal: str = DEFAULT_SEAL) -> dict[str, Any]:
    ...
    try:
        ...
        router, alarm, generic, training = _train_models(train_block)
        _campaign_rows(eligible, raw_rows, router=router, alarm=alarm, generic=generic)
    ...
    base["per_seed"] = _recompute_per_seed(raw_rows)
    base["descriptive"] = _descriptive_block(raw_rows)
