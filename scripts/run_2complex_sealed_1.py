#!/usr/bin/env python3
"""Sealed runner for experiment ``universa-router-2complex-sealed-1``.

The frozen design: one :class:`universa.router.StructureRouter`
(``feature_dim=18``, ``hidden_dim=64``) trained on the 18-dim max-degree
NO-ANCHOR degradation profiles of 2-complex switch instances over the
train seed block 70001..70200 x operating fractions {0.2, 0.3, 0.4, 0.5}
(one replicate, index 0, per row; 800 rows), then compared on the sealed
eval seed block 80101..80136 x held-out fractions {0.6, 0.7, 0.8} against
TWO baselines: the polluted argmin observed-residual oracle (argmin of
the operating-fraction max-degree residual column) and, descriptively,
the non-learned grid-mean ``log1p`` max-degree-profile heuristic. Every
observation is a :class:`universa.partial2.TwoComplexObservationModel`
draw with ``mask_fraction = 0.25`` (edge columns of ``B1`` and the
matching rows of ``B2`` removed, consistently) AND dual sign corruption
(``corrupt_b1_fraction = corrupt_b2_fraction = g`` at grid point ``g``);
the profile grid 0.2..0.9 by 0.1 EXCLUDES 0.0 — no clean column exists
anywhere. Each data row (seed, fraction, replicate) gets
``observation_seed = subseed(seed, "sealed-replicate", str(fraction),
str(replicate_index))``, driving the mask draw AND both nested corruption
draws (``partial2-mask``, ``partial2-b1``, ``partial2-b2``); the SAME
seed feeds all arms within a row (exactly paired), with R=4 replicates
per eval (seed, fraction). The per-(seed, fraction) candidate permutation
is ``subseed(seed, "router-v2-permutation", f"{fraction:.6f}")``,
shared across the row's replicates, with audited labels. The generator
seed is the inference unit for the four one-sided Bonferroni claims
(per-claim alpha 0.05/4 = 0.0125).

**Retention (frozen).** Every eval raw row retains, per candidate, the
FULL dual-degree residual profile — both observed commutation residuals
(degree 1 and degree 2) at all 8 grid points, 16 raw floats in permuted
candidate order — plus the operating-fraction max-degree residual. The
oracle arm's per-row bits are recomputable from the retained operating
residuals and the heuristic arm's per-row bits from the retained full
profiles: BOTH non-learned arms recompute from the raw rows alone, and
the audit block does exactly that. Only the learned arm needs the
retained model (hash-pinned in ``training.model_state_sha256``).

**Declared caveat (frozen).** The comparison is asymmetric by design: the
learned router integrates the full max-degree degraded profile; the
oracle reads only one polluted column (the operating fraction's
max-degree residual). The heuristic baseline in H4 is the profile-shape
...
(``docs/08-router-2complex-seal.json``) have been committed and pushed.
...
EXPERIMENT_ID = "universa-router-2complex-sealed-1"
RESULT_SCHEMA = "universa-router-2complex-sealed-result/1"
...
PROTOCOL = "docs/07-sealed-router-2complex-protocol.md"
RUNNER_SOURCE = "scripts/run_2complex_sealed_1.py"
DEFAULT_SEAL = "docs/08-router-2complex-seal.json"
SEAL_SCHEMA = "universa-seal/3"

PROTOCOL_SHA256 = (
    "792866b64cf5f18b7dcc701c4277d6e1930d5d8cfd0891bfc649337996a08cce"
)
...
# Frozen task family: 2-complex switch instances, K = 4 candidates with the
...
TRAIN_SEEDS = tuple(range(70001, 70201))  # 70001..70200
SEALED_EVAL_SEEDS = tuple(range(80101, 80137))  # 80101..80136
...
FEATURE_DIM = 18  # 8 max-degree profile + 7 slopes + 3 masked structural dims
HIDDEN_DIM = 64
EPOCHS = 150
LEARNING_RATE = 1e-3
TORCH_SEED = 4242
...
# H1..H3, ``d_heur`` (learned minus the non-learned grid-mean max-degree
# profile heuristic) for H4.
_CLAIM_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        "id": "h1-2complex-0.7-primary",
        "fraction": 0.7,
        "threshold": 0.0,
        "role": "primary",
        "baseline": "oracle",
        ...
        "description": (
            ...
            "oracle (operating-fraction max-degree column) at the held-out "
            "fraction 0.7"
        ),
    },
    {
    ...
    },
    {
        ...
        "difference": "d_heur",
        "description": (
            "the learned router beats the non-learned grid-mean log1p "
            "max-degree profile heuristic at the held-out fraction 0.7"
        ),
    },
)
...
DECLARED_CAVEAT = (
    ...
    "the full max-degree degraded profile; the oracle reads only one "
    "polluted column (the operating fraction's max-degree residual). The "
    "heuristic baseline in H4 is the profile-shape non-learned comparator. "
...
class _ReplicateRow:
    ...
    ``features`` is the permuted (K, FEATURE_DIM) float64 max-degree
    no-anchor degradation-profile block exactly as
    :func:`two_complex_candidate_features` computes it (per-candidate
    ``log1p`` of the max of both observed commutation residuals over the
    0.2..0.9 grid under mask_fraction 0.25 and dual sign corruption,
    first-difference slopes, masked structural dims). ``residual_profiles``
    holds, per candidate, the
    raw dual-degree profile (both degree residuals at all grid points, 16
    floats, permuted order); ``operating_residuals`` the per-candidate
    max-degree residual at the operating fraction — the column the polluted
    oracle reads. All derive from ONE shared observation draw (the same seed
    drives the mask draw and both nested corruption draws), so the arms are
    exactly paired.
    """
...
def _replicate_observation_seed(seed: int, fraction: float, replicate: int) -> int:
    ...
    the mask draw AND both nested corruption draws (``partial2-mask``,
    ``partial2-b1``, ``partial2-b2``) of every
    :class:`universa.partial2.TwoComplexObservationModel` the row constructs.
    """
    if replicate < 0:
...
def _feature_names(profile_grid: Any = PROFILE_GRID) -> tuple[str, ...]:
    ...
    ``log1p`` of the max-degree observed commutation residual at every grid
    point, then the first-difference slopes (named by the grid fractions they
    span), then the masked structural dims — ``V'``, the kept edge count, and
    ``F'`` — read off the masked (uncorrupted) observation.
    """
    ...
    return deg_max + slopes + structural


def two_complex_candidate_features(
    chain_map: ChainMap,
    candidate: Any,
    observation_seed: int,
    ...
    """Max-degree no-anchor feature vector for one 2-complex candidate.

    Returns ``(features, deg1_residuals, deg2_residuals)``: the
    FEATURE_DIM-wide float64 feature vector laid out as :func:`_feature_names`
    plus the RAW per-degree residuals, so the row builder can retain the full
    dual-degree profile (the retention contract: both non-learned arms
    recompute from raw rows alone). The feature profile itself is the
    max-degree residual per grid point — the frozen score of the protocol.

    The candidate is observed through
    :class:`universa.partial2.TwoComplexObservationModel` draws with BOTH
    ``mask_fraction`` (edge columns of ``B1`` and rows of ``B2`` removed,
    consistently) and ``corrupt_b1_fraction = corrupt_b2_fraction = g`` for
    every grid fraction ``g``, under the shared ``observation_seed`` — one
    draw per (row, grid point), reused across candidates. The mask draw is
    fixed per row (constant ``mask_fraction``, seed-derived permutation), so
    the same edges are missing at every grid point and each corruption sweep
    is nested per fraction (prefixes of one master permutation per degree).
    Every profile entry is ``log1p`` of the max-degree
    :func:`universa.partial2.observed_commutation_residuals` of the planted
    chain map against the observed candidate — polluted at every grid point,
    with no exact anchor anywhere.

    The structural dims (``V'``, kept edge count, ``F'``) are read off the
    MASKED observed pair (no corruption, the row-fixed degraded operator):
    every feature is computed against a degraded operator.
    """
    grid = _validated_profile_grid(profile_grid)
    if not isinstance(chain_map, ChainMap):
        raise ValueError("chain_map must be a ChainMap instance")
    if candidate.top_degree != 2:
        raise ValueError("routing candidates are 2-complexes")
    ...
    pairs = [
        observed_commutation_residuals(
            ...
            TwoComplexObservationModel(
                ...
                corrupt_b1_fraction=g,
                corrupt_b2_fraction=g,
            ).observe(),
        )
        for g in grid
    ]
    deg1 = tuple(float(pair[0]) for pair in pairs)
    deg2 = tuple(float(pair[1]) for pair in pairs)
    maxdeg = tuple(max(pair) for pair in pairs)
    profile = np.log1p(np.asarray(maxdeg, dtype=np.float64))
    slopes = np.diff(profile)
    masked = TwoComplexObservationModel(
        candidate, observation_seed, mask_fraction=mask_fraction
    ).observe()
    num_vertices, kept_edges = masked.b1.shape
    num_faces = masked.b2.shape[1]
    ...
    return features, deg1, deg2


def _undegraded_misfits(instance: Any) -> tuple[tuple[float, float], ...]:
    """Per-candidate (degree-1, degree-2) clean commutation residuals.

    The sole eligibility audit: deterministic per seed, computed once per
    seed, purely as bookkeeping for seed accounting and label certification
    — it never enters features (every feature is computed against a degraded
    operator).
    """
    misfits = []
    for candidate in instance.candidates:
        ...
        if len(residuals) != 2:
            raise DesignFailureError(
                f"seed {instance.seed}: expected 2 commutation residuals "
                f"(a 2-complex), got {len(residuals)}",
                seed=instance.seed,
            )
        misfits.append((float(residuals[0]), float(residuals[1])))
...
def _build_replicate_row(
    ...
    *,
    ...
    dual-degree profile for the heuristic, and the operating-fraction
    max-degree residuals for the oracle, from ONE shared observation draw.

    The feature block is computed exactly as
    :func:`two_complex_candidate_features` computes it — per candidate under
    the row's shared observation seed with ``mask_fraction = 0.25`` and the
    nested dual-corruption sweep over the 0.2..0.9 grid — permuted by the
    ...
    try:
        per_candidate = [
            two_complex_candidate_features(
                instance.chain_map,
                candidate,
                observation_seed,
                PROFILE_GRID,
    ...
    if with_residuals:
        ...
        residual_profiles = tuple(
            per_candidate[p][1] + per_candidate[p][2] for p in permutation
        )
        operating_residuals = tuple(
            float(
                max(
                    per_candidate[p][1][grid_index],
                    per_candidate[p][2][grid_index],
                )
            )
...
def _build_train_block() -> tuple[np.ndarray, np.ndarray]:
    ...
    eligibility audit on the UNDEGRADED instance applies to every train seed
    at BOTH degrees; any violation is a whole-run design failure, never a
    dropped row.
    """
    ...
    for seed in TRAIN_SEEDS:
        ...
        for degree, value in enumerate(misfits[0], start=1):
            if value > AUDIT_TOL:
                raise DesignFailureError(
                    f"train seed {seed}: true candidate degree-{degree} "
                    f"misfit {value:.3e} on the undegraded instance exceeds "
                    f"{AUDIT_TOL}"
                )
        decoy_floor = min(max(pair) for pair in misfits[1:])
        if decoy_floor <= AUDIT_TOL:
            raise DesignFailureError(
                f"train seed {seed}: a decoy misfit {decoy_floor:.3e} on "
                f"the undegraded instance is at or below {AUDIT_TOL} at "
                "both degrees; ground truth is not separable"
            )
...
def _oracle_index(operating_residuals: Any) -> int:
    ...
    max-degree residual column (log1p is strictly increasing, so this is
    exactly argmin of the profile column the oracle reads)."""
...
def _heuristic_index(residual_profiles: Any) -> int:
    ...
    uniform grid-mean of the ``log1p`` max-degree residual profile — the
    simplest whole-trajectory integration with no learning and no
    operating-fraction choice. Reads only the retained raw dual-degree
    profiles (16 floats per candidate)."""
    grid = len(PROFILE_GRID)
    scores = []
    for profile in residual_profiles:
        values = np.asarray(profile, dtype=np.float64)
        if values.shape != (2 * grid,):
            raise DesignFailureError(
                f"a retained residual profile has width {values.shape}, "
                f"expected {(2 * grid,)}"
            )
        max_degree = np.maximum(values[:grid], values[grid:])
        scores.append(float(np.log1p(max_degree).mean()))
...
def _score_row(
    ...
    argmin over the per-candidate max-degree residuals at the operating
    fraction (the row's profile column; log1p is strictly increasing, so
    this is exactly argmin of the max-degree misfit). Heuristic
    (descriptive, non-learned): argmin over candidates of the uniform
    grid-mean of the ``log1p`` max-degree profile. All three read the same
...
def _recompute_paired(raw_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    operating-fraction max-degree residuals and the heuristic arm's bits
    from the retained full dual-degree profiles (16 floats per candidate) —
    ...
    *,
    ...
    block: dict[str, Any] = {
        "recompute_scope": (
            ...
            "max-degree residuals; the heuristic arm's per-row bits are "
            "recomputed from the retained per-row full dual-degree residual "
            "profiles (both degree residuals at all 8 grid points, 16 "
            "floats per candidate) — BOTH non-learned arms recompute from "
            "the raw rows alone; the learned arm's per-row bits are "
            "recomputable only with the retained model (hash-pinned in "
            "training.model_state_sha256); this block recomputes the "
            "aggregation from the retained raw rows"
        ),
...
def _design_record() -> dict[str, Any]:
    return {
        ...
        "task_family": (
            "2-complex switch instances via "
            "universa.complexes2.make_two_complex_switch_instance("
            "num_vertices=8, num_edges=14, num_classes=6, num_decoys=3); "
            "K=4 candidates, true target at index 0 pre-permutation"
        ),
        "observation_degradation": (
            "universa.partial2.TwoComplexObservationModel with "
            "mask_fraction = 0.25 (edge columns of B1 and the matching "
            "rows of B2 removed, consistently) AND dual sign corruption "
            "(corrupt_b1_fraction = corrupt_b2_fraction = g) at every grid "
            "point; the profile grid 0.2..0.9 by 0.1 excludes 0.0 — no "
            "clean column exists anywhere (the no-anchor regime, lifted to "
            "two degrees)"
        ),
        "feature": (
            "the 18-dim max-degree no-anchor degradation-profile feature "
            "vector exactly as this runner's two_complex_candidate_features "
            "computes it: log1p of the max of the degree-1 and degree-2 "
            "observed commutation residuals at the 8 grid points under the "
            "row's observation draw (8 columns), the 7 first-difference "
            "slopes, and 3 masked structural dims (V', kept edge count, F') "
            "read off the masked observation"
        ),
        ...
        "replicate_subseed": (
            ...
            "both nested corruption draws (partial2-mask, partial2-b1, "
            "partial2-b2); the same observation_seed feeds all arms within "
            "a row (paired)"
        ),
        "permutation": (
            "per (seed, fraction) from subseed(seed, "
            "'router-v2-permutation', f'{fraction:.6f}'), shared "
            "across the row's replicates, with audited labels"
        ),
        "raw_row_retention": (
            "each eval raw row retains, per candidate, the full "
            "dual-degree residual profile (both observed commutation "
            "residuals at all 8 grid points; 16 raw floats, permuted "
            "candidate order) plus the operating-fraction max-degree "
            "residual, so the oracle AND heuristic arms' per-row bits are "
            "recomputable from the raw rows alone (the audit block "
            "recomputes them); only the learned arm needs the retained "
            "hash-pinned model"
        ),
        "training": {
        ...
        "baselines": {
            "oracle": (
                ...
                "operating-fraction max-degree residual column (max of the "
                "two degree residuals; polluted at every fraction — no "
                "clean column exists)"
            ),
            "heuristic": (
                "descriptive non-learned heuristic: argmin of the "
                "grid-mean log1p max-degree profile (the profile-shape "
                "comparator of H4)"
            ),
        },
        ...
        "eligibility_rule": (
            "the instance builds and the clean audit on the UNDEGRADED "
            "instance passes at BOTH degrees (true misfit <= 1e-9 at both "
            "degrees, every decoy > 1e-9 at some degree); deterministic "
            "per seed and never entering features"
        ),
    ...
    try:
        for seed in SEALED_EVAL_SEEDS:
            ...
            for degree, value in enumerate(misfits[0], start=1):
                if value > AUDIT_TOL:
                    problems.append(
                        f"true candidate degree-{degree} misfit "
                        f"{value:.3e} on the undegraded instance exceeds "
                        f"{AUDIT_TOL}"
                    )
            bad_decoys = [
                (index, pair)
                for index, pair in enumerate(misfits[1:])
                if max(pair) <= AUDIT_TOL
            ]
            for index, pair in bad_decoys:
                problems.append(
                    f"decoy {index} misfit {max(pair):.3e} on the "
                    f"undegraded instance is at or below {AUDIT_TOL} at "
                    "both degrees"
                )
            seed_records.append(
                {
                    ...
                    "true_misfits_undegraded": list(misfits[0]),
                    "decoy_misfits_undegraded": [
                        list(pair) for pair in misfits[1:]
                    ],
                }
            )
            if not problems:
