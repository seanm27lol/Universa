#!/usr/bin/env python3
"""Sealed runner for experiment ``universa-router-v2-sealed-1``.

The frozen design: one :class:`universa.router.StructureRouter`
(``feature_dim=18``, ``hidden_dim=64``) trained on the 18-dim NO-ANCHOR
degradation profiles of graph-quotient switch instances over the train seed
block 50001..50200 x operating fractions {0.2, 0.3, 0.4, 0.5} (one replicate,
index 0, per row; 800 rows), then compared on the sealed eval seed block
60101..60136 x held-out fractions {0.6, 0.7, 0.8} against TWO baselines: the
polluted argmin observed-residual oracle (argmin of the operating-fraction
profile column) and, descriptively, the non-learned grid-mean ``log1p``
profile heuristic. Every observation is an
:class:`universa.partial.ObservationModel` draw with ``mask_fraction = 0.25``
AND sign corruption; the profile grid 0.2..0.9 by 0.1 EXCLUDES 0.0 — no
clean column exists anywhere. Each data row (seed, fraction, replicate) gets
``observation_seed = subseed(seed, "sealed-replicate", str(fraction),
str(replicate_index))``, driving BOTH the mask draw and the nested
corruption draws; the SAME seed feeds all arms within a row (exactly
paired), with R=4 replicates per eval (seed, fraction). The per-(seed,
fraction) candidate permutation is
``subseed(seed, "router-v2-permutation", f"{fraction:.6f}")``, shared across
the row's replicates, with audited labels. The generator seed is the
inference unit for the four one-sided Bonferroni claims (per-claim alpha
0.05/4 = 0.0125).

**Declared caveat (frozen).** The comparison is asymmetric by design: the
learned router integrates the full degraded profile; the oracle reads only
one polluted column (the operating fraction). The heuristic baseline in H4
is the profile-shape non-learned comparator. Demo-scale numbers are not
evidence.

This runner must not be executed on the sealed eval seed block until the
protocol, this runner, and the seal record (``docs/05-router-v2-seal.json``)
...
EXPERIMENT_ID = "universa-router-v2-sealed-1"
RESULT_SCHEMA = "universa-router-v2-sealed-result/1"
...
PROTOCOL = "docs/04-sealed-router-v2-protocol.md"
RUNNER_SOURCE = "scripts/run_router_v2_sealed_1.py"
DEFAULT_SEAL = "docs/05-router-v2-seal.json"
SEAL_SCHEMA = "universa-seal/2"

PROTOCOL_SHA256 = (
    "2c78f26530f150a9a7534d951069f131c4ca833142be269b895ddd85a14d0378"
)
...
TRAIN_SEEDS = tuple(range(50001, 50201))  # 50001..50200
SEALED_EVAL_SEEDS = tuple(range(60101, 60137))  # 60101..60136
...
# H1..H3, ``d_heur`` (learned minus the non-learned grid-mean profile
# heuristic) for H4.
_CLAIM_DEFINITIONS: tuple[dict[str, Any], ...] = (
    {
        "id": "h1-no-anchor-0.7-primary",
        "fraction": 0.7,
        "threshold": 0.0,
        "role": "primary",
        "baseline": "oracle",
    ...
    },
    {
        ...
        "description": (
            "the learned router beats the polluted argmin observed-residual "
            "oracle at the held-out fraction 0.8"
        ),
    },
    {
        "id": "h4-no-anchor-heuristic-0.7",
        "fraction": 0.7,
        "threshold": 0.0,
        "role": "secondary",
        "baseline": "heuristic",
        ...
        "description": (
            ...
            "profile heuristic at the held-out fraction 0.7"
        ),
    },
)
...
DECLARED_CAVEAT = (
    ...
    "the full degraded profile; the oracle reads only one polluted column "
    "(the operating fraction). The heuristic baseline in H4 is the "
    "profile-shape non-learned comparator. Demo-scale numbers are not "
    "evidence."
)
...
class _ReplicateRow:
    ...
    degradation-profile block exactly as
    :func:`universa.router_v2.no_anchor_candidate_features` computes it
    (per-candidate ``log1p`` observed-misfit profiles over the 0.2..0.9 grid
    under mask_fraction 0.25, slopes, masked structural dims), and
    ``observed_residuals`` the raw per-candidate misfits at the operating
    fraction in the same permuted order — the column the polluted oracle
    reads. Both derive from ONE shared observation draw (the same seed
    drives the mask draw and the nested corruption draws), so the arms are
    exactly paired.
    """
...
def _replicate_observation_seed(seed: int, fraction: float, replicate: int) -> int:
    ...
    BOTH the mask draw and the nested corruption draws of every
    :class:`universa.partial.ObservationModel` the row constructs.
    """
    if replicate < 0:
...
def _undegraded_misfits(instance: Any) -> tuple[float, ...]:
    """Per-candidate clean commutation residual on the UNDEGRADED instance.
...
def _build_replicate_row(
    ...
    *,
    ...
    """One paired replicate row: features for the learned arm and the
    heuristic, residuals for the oracle, from ONE shared observation draw.

    The feature block is computed exactly as
    :func:`universa.router_v2.no_anchor_candidate_features` computes it —
    per candidate under the row's shared observation seed with
    ``mask_fraction = 0.25`` and the nested corruption sweep over the
    0.2..0.9 grid — permuted by the per-(seed, fraction) permutation from
    ``subseed(seed, "router-v2-permutation", f"{fraction:.6f}")`` (shared
    across the row's replicates). The eligibility audit is per-seed on the
    UNDEGRADED instance (see :func:`_undegraded_misfits`) and is not repeated
    here; the row-level fail-closed checks are candidate count, grid
    membership, block shape, and finiteness.
    """
...
def _build_train_block() -> tuple[np.ndarray, np.ndarray]:
    ...
    observation draw and the per-(seed, fraction) permutation — NOT via
    :func:`universa.router_v2.build_no_anchor_dataset`, whose per-instance
    draw is the artifact this design fixes. The per-seed eligibility audit
    on the UNDEGRADED instance applies to every train seed; any violation is
    a whole-run design failure, never a dropped row.
    """
    ...
    for seed in TRAIN_SEEDS:
        ...
        if misfits[0] > AUDIT_TOL:
            raise DesignFailureError(
                f"train seed {seed}: true candidate misfit {misfits[0]:.3e} "
                f"on the undegraded instance exceeds {AUDIT_TOL}"
            )
...
def _score_row(
    ...
    this is exactly argmin of the misfit). Heuristic (descriptive,
    non-learned): argmin over candidates of the uniform grid-mean of the
    ``log1p`` profile columns. All three read the same row, so the
    correctness triple is exactly paired.
    ...
    heuristic_index = int(
        np.argmin(features[:, : len(PROFILE_GRID)].mean(axis=1))
    )
    return (
    ...
    *,
    ...
    block: dict[str, Any] = {
        "recompute_scope": (
            ...
            "recomputable from the retained per-row observed residuals; the "
            "learned arm's per-row bits are recomputable only with the "
            "retained model (hash-pinned in training.model_state_sha256); "
            "the heuristic arm's per-row bits are recomputable from the "
            "retained observation draws (observation_seed and permutation "
            "are stored per row) via the certified feature machinery, not "
            "from the raw rows alone; this block recomputes the aggregation "
            "from the retained raw rows"
        ),
...
def _design_record() -> dict[str, Any]:
    return {
        ...
        "observation_degradation": (
            "universa.partial.ObservationModel with mask_fraction = 0.25 "
            "(edge columns removed) AND sign corruption at every grid "
            "point; the profile grid 0.2..0.9 by 0.1 excludes 0.0 — no "
            "clean column exists anywhere (the no-anchor regime)"
        ),
        "feature": (
            "the 18-dim no-anchor degradation-profile feature vector "
            "exactly as universa.router_v2.no_anchor_candidate_features "
            "computes it: log1p observed misfit at the 8 grid points under "
            "the row's observation draw, 7 first-difference slopes, and 3 "
            "masked structural dims (V', kept edge count, masked cycle "
            "nullity)"
        ),
        ...
        "replicate_subseed": (
            ...
            "str(replicate_index)) per data row, driving BOTH the mask "
            "draw and the nested corruption draws; the same "
            "observation_seed feeds all arms within a row (paired)"
        ),
        "permutation": (
            "per (seed, fraction) from subseed(seed, "
            "'router-v2-permutation', f'{fraction:.6f}'), shared across "
            "the row's replicates, with audited labels"
        ),
        "training": {
        ...
        "baselines": {
            "oracle": (
                ...
                "operating-fraction profile column (polluted at every "
                "fraction — no clean column exists)"
            ),
            "heuristic": (
                "descriptive non-learned heuristic: argmin of the "
                "grid-mean log1p profile (the profile-shape comparator "
                "of H4)"
            ),
        },
        ...
        "eligibility_rule": (
            ...
            "instance passes (true misfit <= 1e-9, every decoy > 1e-9); "
    ...
    try:
        for seed in SEALED_EVAL_SEEDS:
            ...
            if true_misfit > AUDIT_TOL:
                problems.append(
                    f"true candidate misfit {true_misfit:.3e} on the "
            ...
            for index, misfit in bad_decoys:
                problems.append(
                    f"decoy {index} misfit {misfit:.3e} on the undegraded "
            ...
            seed_records.append(
                {
                    ...
                    "true_misfit_undegraded": true_misfit,
                    "decoy_misfits_undegraded": list(decoy_misfits),
